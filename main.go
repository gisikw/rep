...
func main() {
	cfg, err := LoadConfig()
	if err != nil {
		fmt.Fprintf(os.Stderr, "crane: config error: %v\n", err)
		os.Exit(1)
	}
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "Usage: crane [flags] [prompt]\n")
	...
	if opts.Prompt == "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "crane: failed to read stdin: %v\n", err)
			os.Exit(1)
		}
	...
	if opts.Prompt == "" {
		fmt.Fprintf(os.Stderr, "crane: no prompt provided\n")
		os.Exit(1)
	}
	...
	if err := cmd.Run(); err != nil {
		if exitErr, ok := err.(*os.PathError); ok {
			fmt.Fprintf(os.Stderr, "crane: provider %q not found: %v\n", provider, exitErr)
			os.Exit(1)
		}
